...
class BenachrichtigungenSeite extends StatefulWidget {
  ...
  @override
  State<BenachrichtigungenSeite> createState() => _BenachrichtigungenSeiteState();
}
...
class _BenachrichtigungenSeiteState extends State<BenachrichtigungenSeite> {
  ...
  String _kategorieAnzeige(String kategorie) {
    if (kategorie == 'Autos' || kategorie == 'Motorräder' || kategorie == 'Motorrad') return 'Auto & Motor';
  ...
  int _zahl(String text) {
    return int.tryParse(
          text.replaceAll('€', '').replaceAll('.', '').replaceAll(',', '')
              .replaceAll('/Tag', '').replaceAll('pro Tag', '').trim(),
        ) ?? 0;
  }

  Future<void> _chatAlsGelesenMarkieren(DocumentReference ref, String userId) async {
    await ref.set({
      'ungelesenFuer': '',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: ListView(
          padding: EdgeInsets.fromLTRB(breit ? 46 : 16, 18, breit ? 46 : 16, 24),
          children: [
            _header(context),
      ...
      child: Row(
        children: [
          Container(
            ...
            height: 44,
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [Color(0xff5b2cff), Color(0xff7a5cff)]),
  ...
  Widget _heroKarte(User? user) {
    if (user == null) {
      return _heroInhalt(chats: 0, ungelesen: 0, favoriten: 0);
    }
      ...
      builder: (context, chatSnapshot) {
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, favSnapshot) {
            ...
            return _heroInhalt(chats: chats.length, ungelesen: ungelesen, favoriten: favoriten);
          },
        );
      },
    );
  ...
  Widget _heroInhalt({required int chats, required int ungelesen, required int favoriten}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x225b2cff), blurRadius: 18, offset: Offset(0, 8))],
      ),
      child: Row(
        children: [
          Container(
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text('$chats', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900)),
              ],
            ),
          ),
        ],
  ...
  Widget _chatBereich(BuildContext context, String userId) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        chats.sort((a, b) {
          final aZeit = _datum((a.data() as Map)['aktualisiertAm']);
          final bZeit = _datum((b.data() as Map)['aktualisiertAm']);
          ...
          child: chats.isEmpty
              ...
              : Column(
                  children: chats.take(8).map((doc) {
                    final daten = doc.data() as Map<String, dynamic>;
                    return _chatKarte(context: context, chatRef: doc.reference, daten: daten, userId: userId);
                  }).toList(),
                ),
        );
      },
  ...
  Widget _favoritenBereich(BuildContext context, String userId) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        favoriten.sort((a, b) {
          final aZeit = _datum((a.data() as Map)['gespeichertAm'] ?? (a.data() as Map)['erstelltAm']);
          final bZeit = _datum((b.data() as Map)['gespeichertAm'] ?? (b.data() as Map)['erstelltAm']);
          if (aZeit == null && bZeit == null) return 0;
          if (aZeit == null) return 1;
          if (bZeit == null) return -1;
          return bZeit.compareTo(aZeit);
        ...
        return _aufklappBereich(
          ...
          hinweis: favoriten.isEmpty ? 'Noch nichts gespeichert' : 'Deine zuletzt gespeicherten Inserate',
          ...
          child: favoriten.isEmpty
              ...
              : Column(
                  children: favoriten.take(8).map((doc) {
                    return _favoritKarte(context, doc.data() as Map<String, dynamic>);
                  }).toList(),
                ),
        );
      },
  ...
  Widget _personalisierteInserate(BuildContext context, String? userId) {
    if (userId == null) return _alleInserate(context, bevorzugteKategorien: const {});
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance.collection('favoriten').where('userId', isEqualTo: userId).snapshots(),
      builder: (context, favSnapshot) {
        final kategorieZaehler = <String, int>{};
        for (final doc in favSnapshot.data?.docs ?? []) {
          final daten = doc.data() as Map<String, dynamic>;
          final kat = _kategorieAnzeige(_text(daten, ['produktKategorie', 'kategorie', 'category']));
          if (kat.trim().isEmpty) continue;
          kategorieZaehler[kat] = (kategorieZaehler[kat] ?? 0) + 1;
        }
        final top = (kategorieZaehler.entries.toList()..sort((a, b) => b.value.compareTo(a.value))).take(3).map((e) => e.key).toSet();
        return _alleInserate(context, bevorzugteKategorien: top);
      },
    );
  }

  Widget _alleInserate(BuildContext context, {required Set<String> bevorzugteKategorien}) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final alle = snapshot.data!.docs.map((doc) => Produkt.fromFirestore(doc)).where((p) {
          if (bevorzugteKategorien.isEmpty) return true;
          return bevorzugteKategorien.contains(_kategorieAnzeige(p.kategorie));
        }).toList();

        final top = [...alle]..sort((a, b) => _zahl(a.preis).compareTo(_zahl(b.preis)));
        final neu = [...alle].reversed.toList();
        ...
        return Column(
          ...
          children: [
            ...
            _swipeBereich(titel: 'Top Inserate für dich', icon: Icons.local_fire_department_outlined, produkte: top.take(12).toList()),
            const SizedBox(height: 16),
            _swipeBereich(titel: 'Neue Inserate für dich', icon: Icons.new_releases_outlined, produkte: neu.take(12).toList()),
          ],
        );
      },
    );
  ...
  Widget _swipeBereich({required String titel, required IconData icon, required List<Produkt> produkte}) {
    return Container(
      width: double.infinity,
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text('$titel (${produkte.length})',
                    style: const TextStyle(color: Color(0xff050b2c), fontSize: 17, fontWeight: FontWeight.w900)),
              ),
          ...
          if (produkte.isEmpty)
            ...
            SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) => _produktKarte(context, produkte[index]),
              ),
            ),
        ],
      ),
            ...
            ClipRRect(
              ...
              child: SizedBox(
                ...
                child: produkt.bild.trim().isEmpty
                    ? Container(color: const Color(0xfff1edff), child: Icon(produkt.icon, color: const Color(0xff5b2cff), size: 38))
                    : Image.network(produkt.bild, fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => Container(color: const Color(0xfff1edff), child: Icon(produkt.icon, color: const Color(0xff5b2cff), size: 38))),
              ),
            ),
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(produkt.titel.isEmpty ? 'Inserat' : produkt.titel,
                      maxLines: 2, overflow: TextOverflow.ellipsis,
                      style: const TextStyle(color: Color(0xff050b2c), fontWeight: FontWeight.w900, height: 1.15)),
                  const SizedBox(height: 6),
                  Text(produkt.preis.endsWith('€') ? produkt.preis : '${produkt.preis} €',
                      maxLines: 1, overflow: TextOverflow.ellipsis,
                      style: const TextStyle(color: Color(0xff5b2cff), fontWeight: FontWeight.w900, fontSize: 15)),
                  const SizedBox(height: 6),
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(produkt.ort.isEmpty ? 'Österreich' : produkt.ort,
                            maxLines: 1, overflow: TextOverflow.ellipsis,
              ...
              Stack(
                ...
                children: [
                  CircleAvatar(
                    ...
                    child: Text(initial, style: const TextStyle(color: Color(0xff5b2cff), fontSize: 21, fontWeight: FontWeight.w900)),
                  ),
                  if (ungelesen)
                    Positioned(
                      right: -2, top: -2,
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Expanded(
                          child: Text(produktTitel.isEmpty ? 'Inserat' : produktTitel,
                              maxLines: 1, overflow: TextOverflow.ellipsis,
                              style: TextStyle(color: const Color(0xff050b2c), fontSize: 15,
                                  fontWeight: ungelesen ? FontWeight.w900 : FontWeight.w800)),
                        ),
                        if (zeit.isNotEmpty)
                          Text(zeit, style: const TextStyle(color: Color(0xff74788d), fontSize: 12, fontWeight: FontWeight.w800)),
                      ],
                    ),
                    const SizedBox(height: 4),
                    Text(anderesEmail.isEmpty ? 'Kontakt' : anderesEmail,
                        maxLines: 1, overflow: TextOverflow.ellipsis,
                        style: const TextStyle(color: Color(0xff5b2cff), fontSize: 13, fontWeight: FontWeight.w800)),
                    const SizedBox(height: 5),
                    Text(letzteNachricht.isEmpty ? 'Neue Nachricht' : letzteNachricht,
                        maxLines: 1, overflow: TextOverflow.ellipsis,
                        style: const TextStyle(color: Color(0xff74788d), fontSize: 13, fontWeight: FontWeight.w700)),
                  ],
                ),
              ),
  ...
  Widget _favoritKarte(BuildContext context, Map<String, dynamic> daten) {
    ...
    final kategorie = _kategorieAnzeige(_text(daten, ['produktKategorie', 'kategorie', 'category']));
    ...
    return Container(
      ...
      child: InkWell(
        ...
        onTap: () async {
          ...
          final doc = await FirebaseFirestore.instance.collection('inserate').doc(produktId).get();
          if (!context.mounted) return;
          if (!doc.exists) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Dieses Inserat wurde gelöscht.')));
            return;
          }
          Navigator.push(context, MaterialPageRoute(builder: (_) => DetailSeite(produkt: Produkt.fromFirestore(doc))));
        },
        child: Container(
            ...
            children: [
              ClipRRect(
                ...
                child: SizedBox(
                  width: 58, height: 58,
                  child: bild.isEmpty
                      ? Container(color: const Color(0xfff1edff), child: const Icon(Icons.favorite, color: Color(0xff5b2cff)))
                      : Image.network(bild, fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) => Container(color: const Color(0xfff1edff), child: const Icon(Icons.favorite, color: Color(0xff5b2cff)))),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(titel.isEmpty ? 'Favorit' : titel, maxLines: 1, overflow: TextOverflow.ellipsis,
                        style: const TextStyle(color: Color(0xff050b2c), fontWeight: FontWeight.w900)),
                    const SizedBox(height: 4),
                    Text(preisText, maxLines: 1, overflow: TextOverflow.ellipsis,
                        style: const TextStyle(color: Color(0xff5b2cff), fontWeight: FontWeight.w900)),
                    const SizedBox(height: 4),
                    Text([ort, kategorie].where((e) => e.trim().isNotEmpty).join(' • '),
                        maxLines: 1, overflow: TextOverflow.ellipsis,
                        style: const TextStyle(color: Color(0xff74788d), fontSize: 12, fontWeight: FontWeight.w700)),
                  ],
                ),
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  Container(
                    width: 42, height: 42,
                    decoration: BoxDecoration(color: iconFarbe.withOpacity(0.11), borderRadius: BorderRadius.circular(14)),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (hinweis.trim().isNotEmpty) ...[
                          ...
                          Text(hinweis, maxLines: 1, overflow: TextOverflow.ellipsis,
                              style: const TextStyle(color: Color(0xff74788d), fontSize: 12, fontWeight: FontWeight.w700)),
                        ],
                      ],
                    ),
                  ),
                  Icon(offen ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
                      color: const Color(0xff5b2cff), size: 28),
                ],
              ),
            ),
          ),
  ...
  Widget _fehler(String text) {
    return Container(
      width: double.infinity, padding: const EdgeInsets.all(22),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.red.withOpacity(0.25))),
      child: Text(text, textAlign: TextAlign.center,
